...
// Mapa para armazenar dados de paginação com timeout de 30 minutos
    ...
    try {
        ...
        try {
            ...
            const embed = new EmbedBuilder()
                ...
                .setTimestamp()
                .setFooter({
                    text: `🔒 Visível apenas para você | Use os botões para navegar`
                });
                    ...
                    if (showStats) {
                        ...
                        cardsText = sortedCards
                            .map((card, index) => {
                                ...
                                return `[${card.position}] ${generateHaxBall(card.rarity)} ${card.player_name} ${card.avatar || card.template_avatar || '⚽'} - ${card.overall_rating} ⭐ \`#${card.id}\`\n   ${statsText}`;
                            })
                            .join('\n');
                    } else {
                        ...
                        cardsText = sortedCards
                            .map((card) => {
                                ...
                                return `[${card.position}] ${generateHaxBall(card.rarity)} ${card.player_name} ${card.avatar || card.template_avatar || '⚽'} - ${card.overall_rating} ⭐${stats} \`#${card.id}\``;
                            })
                            .join('\n');
                    }
            ...
            await interaction.editReply({
                content: '❌ **Erro ao buscar suas cartas!**\n' +
...
// Função para lidar com navegação de páginas - versão para minhascartas
const handlePaginationInteractionMinhas = async (interaction) => {
    ...
    if (!data) {
        return await interaction.reply({
            content: '⏰ **PAGINAÇÃO EXPIRADA!**\n\nEsta paginação não é mais válida (30 minutos). Use o comando novamente.',
            ephemeral: true
        });
    }
...
module.exports = {
    data: new SlashCommandBuilder()
        ...
        .setDescription('🎴 Ver suas cartas com filtros e paginação')
        ...
        .addBooleanOption(option =>
            option.setName('stats')
                .setDescription('Mostrar estatísticas detalhadas')
                .setRequired(false))
